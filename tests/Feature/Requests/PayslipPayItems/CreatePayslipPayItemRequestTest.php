...
use HWafeq\LaravelWafeq\Requests\PayslipPayItems\CreatePayslipPayItemRequest;
...
it('validates a fully-populated pay item payload', function () {
    ...
    $request = CreatePayslipPayItemRequest::create('/payslips/ps_1/pay-items/', 'POST', $payload);
...
it('rejects a pay item payload missing required fields', function () {
    $request = CreatePayslipPayItemRequest::create('/payslips/ps_1/pay-items/', 'POST', []);
    $request->setContainer($this->app);
    $request->setRedirector($this->app->make(Redirector::class));
