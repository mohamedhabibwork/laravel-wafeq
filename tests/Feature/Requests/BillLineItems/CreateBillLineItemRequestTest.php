...
use HWafeq\LaravelWafeq\Requests\BillLineItems\CreateBillLineItemRequest;
use Illuminate\Routing\Redirector;
use Illuminate\Support\Facades\Validator;

it('validates a fully-populated bill line item create payload', function () {
    $payload = [
        'account' => 'acc_1',
        'description' => 'Item A',
        'quantity' => '1.00',
        ...
        'amortization' => ['account' => 'amort_acc', 'amount' => '12.00'],
    ];

    $request = CreateBillLineItemRequest::create('/bills/bill_1/line-items/', 'POST', $payload);
...
it('rejects a bill line item payload missing required fields', function () {
    $request = CreateBillLineItemRequest::create('/bills/bill_1/line-items/', 'POST', []);
    $request->setContainer($this->app);
    $request->setRedirector($this->app->make(Redirector::class));
