...
use HWafeq\LaravelWafeq\Requests\CreditNoteLineItems\CreateCreditNoteLineItemRequest;
use Illuminate\Routing\Redirector;
use Illuminate\Support\Facades\Validator;

it('validates a fully-populated credit note line item create payload', function () {
    $payload = [
        'account' => 'acc_1',
        'description' => 'Item A',
        'quantity' => '1.00',
    ...
    $request = CreateCreditNoteLineItemRequest::create('/credit-notes/cn_1/line-items/', 'POST', $payload);
...
it('rejects a credit note line item payload missing required fields', function () {
    $request = CreateCreditNoteLineItemRequest::create('/credit-notes/cn_1/line-items/', 'POST', []);
    $request->setContainer($this->app);
    $request->setRedirector($this->app->make(Redirector::class));
