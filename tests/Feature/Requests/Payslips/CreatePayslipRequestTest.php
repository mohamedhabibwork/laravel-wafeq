...
use HWafeq\LaravelWafeq\Requests\Payslips\CreatePayslipRequest;
...
it('validates a fully-populated payslip payload', function () {
    ...
    $request = CreatePayslipRequest::create('/payslips/', 'POST', $payload);
...
it('rejects a payslip payload missing required fields', function () {
    $request = CreatePayslipRequest::create('/payslips/', 'POST', []);
    $request->setContainer($this->app);
    $request->setRedirector($this->app->make(Redirector::class));
...
it('rejects an invalid payslip_date format', function () {
    $request = CreatePayslipRequest::create('/payslips/', 'POST', []);
    $request->setContainer($this->app);
    $request->setRedirector($this->app->make(Redirector::class));

    $validator = Validator::make(
        ['payslip_date' => '15/01/2026'],
        ['payslip_date' => $request->rules()['payslip_date']],
    );

    expect($validator->fails())->toBeTrue();
});

it('rejects a pay_items entry missing required fields', function () {
    $request = CreatePayslipRequest::create('/payslips/', 'POST', []);
    $request->setContainer($this->app);
    $request->setRedirector($this->app->make(Redirector::class));
    ...
    $payload = [
        'pay_items' => [
            ['amount' => '100.00'], // missing account + description
        ],
    ];

    $validator = Validator::make($payload, $request->rules());
    expect($validator->fails())->toBeTrue()
        ->and($validator->errors()->has('pay_items.0.account'))->toBeTrue()
        ->and($validator->errors()->has('pay_items.0.description'))->toBeTrue();
});
