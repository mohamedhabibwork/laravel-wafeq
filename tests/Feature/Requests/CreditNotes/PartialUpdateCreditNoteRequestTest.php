...
use HWafeq\LaravelWafeq\Requests\CreditNotes\PartialUpdateCreditNoteRequest;
use Illuminate\Routing\Redirector;
use Illuminate\Support\Facades\Validator;

it('validates a partial-update payload that only sets one field', function () {
    $payload = ['credit_note_number' => 'CN-2026-003'];

    $request = PartialUpdateCreditNoteRequest::create('/credit-notes/abc/', 'PATCH', $payload);
...
it('treats an empty partial-update payload as valid', function () {
    $request = PartialUpdateCreditNoteRequest::create('/credit-notes/abc/', 'PATCH', []);
    $request->setContainer($this->app);
    $request->setRedirector($this->app->make(Redirector::class));
