...
use HWafeq\LaravelWafeq\Requests\Beneficiaries\CreateBeneficiaryRequest;
...
it('validates a fully-populated beneficiary payload', function () {
    ...
    $request = CreateBeneficiaryRequest::create('/beneficiaries/', 'POST', $payload);
...
it('rejects a beneficiary payload missing required fields', function () {
    $request = CreateBeneficiaryRequest::create('/beneficiaries/', 'POST', []);
    $request->setContainer($this->app);
    $request->setRedirector($this->app->make(Redirector::class));
...
it('rejects an over-long iban', function () {
    $request = CreateBeneficiaryRequest::create('/beneficiaries/', 'POST', []);
    $request->setContainer($this->app);
    $request->setRedirector($this->app->make(Redirector::class));
