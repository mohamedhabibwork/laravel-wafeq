...
use HWafeq\LaravelWafeq\Requests\Invoices\PartialUpdateInvoiceRequest;
use Illuminate\Routing\Redirector;
use Illuminate\Support\Facades\Validator;

it('validates a fully-populated invoice payload for PATCH', function () {
    $payload = [
        'contact' => 'contact_1',
        'currency' => 'SAR',
    ...
    $request = PartialUpdateInvoiceRequest::create('/invoices/inv_1/', 'PATCH', $payload);
...
it('rejects an invoice payload missing required fields on PATCH', function () {
    $request = PartialUpdateInvoiceRequest::create('/invoices/inv_1/', 'PATCH', []);
    $request->setContainer($this->app);
    $request->setRedirector($this->app->make(Redirector::class));
