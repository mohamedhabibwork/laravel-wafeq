...
```bash
php artisan vendor:publish --tag="laravel-wafeq-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="laravel-wafeq-views"
```

## Usage

```php
$laravelWafeq = new HWafeq\LaravelWafeq();
echo $laravelWafeq->echoPhrase('Hello, HWafeq!');
```
