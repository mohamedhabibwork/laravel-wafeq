...
/**
 * FormRequest for `PATCH /accounts/{id}/` — Partial update account.
 *
 * Validation rules are derived from the OpenAPI schema in
 * `wafeq-docs/accounts_partial_update.md`. The `PatchedAccount` schema
 * contains the same read-write fields as `Account` but without the
 ...
 *   - `classification`      enum             — see ClassificationEnum
...
class PartialUpdateAccountRequest extends WafeqFormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // AccountTypeEnum — narrowed to string here.
            'account_type' => ['nullable', 'string'],

            // ClassificationEnum: REVENUE | EXPENSE | ASSET | BANK | LIABILITY | EQUITY
            'classification' => ['nullable', 'string', Rule::enum(Classification::class)],
            ...
            'sub_classification' => ['nullable', 'string', Rule::enum(AccountSubclassification::class)],
        ];
    }
