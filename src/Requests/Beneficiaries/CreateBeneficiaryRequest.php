...
/**
 * FormRequest for `POST /beneficiaries/` — Create beneficiary.
 *
 * Validation rules are derived from the OpenAPI schema in
 * `wafeq-docs/beneficiaries_create.md` (the
 * `api-v1-external-beneficiary-read-write` schema).
 *
 ...
 */
class CreateBeneficiaryRequest extends WafeqFormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // ISO-3166 two-letter country code — full enum list lives
            // in `CountryEnum` (enforced via DTO cast on the way in).
            'country' => ['required', 'string'],

            // ISO-4217 currency code — full enum list lives in
            // `CurrencyEnum` (enforced via DTO cast on the way in).
            'currency' => ['required', 'string'],

            // International Bank Account Number (IBAN).
            'iban' => ['required', 'string', 'max:34'],

            // Beneficiary name (Latin letters, digits, spaces).
