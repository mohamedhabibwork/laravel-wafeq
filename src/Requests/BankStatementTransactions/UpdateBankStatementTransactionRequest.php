 ...
 * `wafeq-docs/bank_accounts_statement_transactions_update.md`. The
 * schema is identical to the create endpoint, so the ruleset matches
 * `CreateBankStatementTransactionRequest` one-to-one.
 *
 * The endpoint's request body shape (read-write fields):
 *
 *   - `amount`            required number  — statement amount
 *   - `bank_reference`    string           — bank's reference id
 *   - `cost_center`       string|null      — optional cost-center ref
 *   - `date`              required date    — statement date (yyyy-mm-dd)
 *   - `description`       string           — transaction description
 *   - `project`           string|null      — optional project ref
 *   - `reference`         string           — internal reference
 *   - `statement_balance` required number  — bank-reported balance
 *
 ...
 */
class UpdateBankStatementTransactionRequest extends WafeqFormRequest
{
