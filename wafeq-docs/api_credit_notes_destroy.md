...
Endpoint for deleting an existing credit note (bulk).
...
```json
{
  ...
  "paths": {
    "/api-credit-notes/{id}/": {
      "delete": {
        "description": "Endpoint for deleting an existing credit note (bulk).",
        "operationId": "api_credit_notes_destroy",
        "parameters": [
          {
        ...
        "summary": "Delete credit note (bulk)",
        "tags": [
          "Credit Notes (Bulk)"
        ]
      }
